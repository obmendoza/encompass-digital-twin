# Slice 2 — Pipeline View Implementation Plan

> **For agentic workers:** Use superpowers:subagent-driven-development or superpowers:executing-plans.

**Goal:** Replace the hardcoded redirect with a pipeline table showing all 12 NQM loans.

**Architecture:** Web-only changes. No core or api modifications.

**Tech Stack:** Next.js 15 + Tailwind + existing Encompass theme classes.
...
packages/web/
├─ app/
│  ├─ page.tsx                          # REPLACE redirect → pipeline server component
│  └─ layout.tsx                        # ADD pipeline shell (titlebar/menu/toolbar, no loan header)
├─ components/
│  └─ encompass/
│     ├─ PipelineTable.tsx              # NEW — client component with sort + filter
│     └─ Toolbar.tsx                    # MODIFY — Pipeline button becomes Link to /
```

---

### Task 1: PipelineTable client component

**Files:**
- Create: `packages/web/components/encompass/PipelineTable.tsx`
...
- [ ] Create the file
- [ ] Commit: `feat(web): PipelineTable component with sort + filter`

---

### Task 2: Pipeline page + Toolbar link

**Files:**
- Replace: `packages/web/app/page.tsx`
- Modify: `packages/web/components/encompass/Toolbar.tsx`

**`packages/web/app/page.tsx`** (full rewrite):
```tsx
...
**`packages/web/components/encompass/Toolbar.tsx`** (add Link):
```tsx
import Link from "next/link";
...
- [ ] Replace page.tsx
- [ ] Modify Toolbar.tsx
- [ ] Commit: `feat(web): pipeline page + toolbar link`

---

### Task 3: Build + smoke

- [ ] Run `corepack pnpm --filter @twin/web exec next build` — expect success
- [ ] Verify / returns 200 with pipeline content
- [ ] Verify clicking a loan row navigates to the correct transmittal page
- [ ] Commit any fixes
- [ ] Tag: `slice-2-complete`
